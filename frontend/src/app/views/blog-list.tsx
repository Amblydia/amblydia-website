				...
				return (
					...
					>
						...
						<div className="flex items-center font-bold mb-2">
							<div className={`${renderTagStyle(article.attributes.tags)}`}>{article.attributes.tags}</div>
						...
						<p className="text-base pr-5">{article.attributes.description}</p>
				...
				return (
					<a href={`/blog/${article.attributes.slug}`} className="grid lg:grid-cols-2 gap-16 col-span-2">
						...
						<div className="text-left">
							<header>
								<div className={renderTagStyle(article.attributes.tags)}>{article.attributes.tags}</div>
								<h2 className="font-bold text-2xl mb-1">{article.attributes.title}</h2>
							</header>
